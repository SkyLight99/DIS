        ...
        for i_val, data_val in enumerate(valid_dataloader):
            ...
            for t in range(hypar["batch_size_valid"]):
                ...
                pred_val = torch.squeeze(F.upsample(torch.unsqueeze(
                    pred_val, 0), (shapes_val[t][0], shapes_val[t][1]), mode='bilinear'))
            ...
            print("[validating: %5d/%5d] val_ls:%f, tar_ls: %f, f1: %f, mae: %f, time: %f" % (i_val, val_num,
                  val_loss / (i_val + 1), tar_loss / (i_val + 1), np.amax(F1[i_test, :]), MAE[i_test], t_end))
    ...
    for epoch in range(epoch_num):  # set the epoch num as 100000
        ...
        for i, data in enumerate(gos_dataloader):
            ...
            if hypar["interm_sup"]:
                ...
                loss2, loss = net.compute_loss_kl(
                    ds, labels_v, dfs, fs, mode='MSE')
            else:
                # forward + backward + optimize
                ds, _ = net(inputs_v)
                loss2, loss = net.compute_loss(ds, labels_v)
...
def valid(net, valid_dataloaders, valid_datasets, hypar, epoch=0):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for i_val, data_val in enumerate(valid_dataloader):
            val_cnt = val_cnt + 1.0
            imidx_val, inputs_val, labels_val, shapes_val = data_val[
                'imidx'], data_val['image'], data_val['label'], data_val['shape']
            ...
            # wrap them in Variable
            if torch.cuda.is_available():
                inputs_val_v, labels_val_v = Variable(inputs_val.cuda(
                ), requires_grad=False), Variable(labels_val.cuda(), requires_grad=False)
            else:
                inputs_val_v, labels_val_v = Variable(
                    inputs_val, requires_grad=False), Variable(labels_val, requires_grad=False)
...
def main(train_datasets,
    ...
    if (hypar["restore_model"] != ""):
        ...
        if torch.cuda.is_available():
            ...
            net.load_state_dict(torch.load(
                hypar["model_path"]+"/"+hypar["restore_model"], map_location="cpu"))
...
if __name__ == "__main__":
    ...
    dataset_tr = {"name": "REFUGE2-TR",
                  "im_dir": "C:/Users/RED LINE/Documents/Dataset/REFUGE2/train/images",
                  "gt_dir": "C:/Users/RED LINE/Documents/Dataset/REFUGE2/train/mask",
                  "im_ext": ".jpg",
                  "gt_ext": ".bmp",
                  "cache_dir": "C:/Users/RED LINE/Documents/Dataset/REFUGE2/train"}

    dataset_vd = {"name": "REFUGE2-VD",
                  "im_dir": "C:/Users/RED LINE/Documents/Dataset/REFUGE2/val/images",
                  "gt_dir": "C:/Users/RED LINE/Documents/Dataset/REFUGE2/val/mask",
                  "im_ext": ".jpg",
                  "gt_ext": ".png",
                  "cache_dir": "C:/Users/RED LINE/Documents/Dataset/REFUGE2/val"}

    dataset_te = {"name": "REFUGE2-TE",
                  "im_dir": "C:/Users/RED LINE/Documents/Dataset/REFUGE2/test/images",
                  "gt_dir": "C:/Users/RED LINE/Documents/Dataset/REFUGE2/test/mask",
                  "im_ext": ".jpg",
                  "gt_ext": ".bmp",
                  "cache_dir": "C:/Users/RED LINE/Documents/Dataset/REFUGE2/test"}
    ...
    # dataset_vd, dataset_te1, dataset_te2, dataset_te3, dataset_te4] # and hypar["mode"] = "valid" for inference,
    valid_datasets = [dataset_te]
    ...
    # in-dicate if activate intermediate feature supervision
    hypar["interm_sup"] = False

    if hypar["mode"] == "train":
        # for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
        hypar["valid_out_dir"] = ""
        # model weights saving (or restoring) path
        hypar["model_path"] = "C:\\Users\\RED LINE\Documents\\University Project Code\\DIS\\saved_models"
        # name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
        hypar["restore_model"] = "gpu_itr_40000_traLoss_0.0577_traTarLoss_0.0089_valLoss_0.258_valTarLoss_0.0298_maxF1_0.9992_mae_0.0019_time_0.020717.pth"
        if hypar["restore_model"] != "":
        ...
        # "../DIS5K-Results-test" ## output inferenced segmentation maps into this fold
        hypar["valid_out_dir"] = "C:\\Users\\RED LINE\Documents\\University Project Code\\DIS\\Results\\40k-iteration-test"
        # load trained weights from this path
        hypar["model_path"] = "C:\\Users\\RED LINE\Documents\\University Project Code\\DIS\\saved_models"
        # "isnet.pth" ## name of the to-be-loaded weights
        hypar["restore_model"] = "gpu_itr_40000_traLoss_0.0577_traTarLoss_0.0089_valLoss_0.258_valTarLoss_0.0298_maxF1_0.9992_mae_0.0019_time_0.020717.pth"
    ...
    # horizontal flip, currently hard coded in the dataloader and it is not in use
    ...
    hypar["batch_size_train"] = 5  # batch size for training
